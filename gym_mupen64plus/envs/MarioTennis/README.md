# MarioTennis Environment

This page describes the MarioTennis environment(s).

## Configuration

A configuration file ([`mario_tennis_config.yml`](mario_tennis_config.yml)) has been provided for this game.

The first configuration setting is `ROM_NAME`. This setting must specify the name of the MarioTennis ROM file (which should be placed in `./gym_mupen64plus/ROMs/`). As mentioned on the top page, links to ROM files will not be provided here.
